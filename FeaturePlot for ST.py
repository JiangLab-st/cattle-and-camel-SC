...
def add_legend(src_img, small_legend_img, base_rate=12):
 ...
 #   new_legend = np.zeros((new_legend_height, new_legend_weight, 3), np.uint8)
    new_legend = np.ones((new_legend_height, new_legend_weight, 3), np.uint8) * 255
...
if __name__ == '__main__':
    ...
    clusters_ori.sort(key=lambda x: len(x[-1])) 
    if special_cluster_dict != {}:  
    ...
    for clu in clusters_ori:
        ...
        clusters.append([int(clu[0].lstrip('cell_')), str(clu[1]), legend[clu[1]]])  
